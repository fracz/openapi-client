...
export class OpenAIService {
  ...
  async countTokens(messages: ChatCompletionMessageParam[], model: string = 'gpt-4.1-nano'): Promise<number> {
  ...
  async completion(config: {
    ...
    const { messages, model = "gpt-4.1-nano", stream = false, jsonMode = false, maxTokens = 4096, temperature = 0 } = config;
    try {
